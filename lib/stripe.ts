...
export async function createPaymentIntent(params: CreatePaymentIntentRequest): Promise<Stripe.PaymentIntent> {
  ...
  try {
    ...
    if (params.shipping && typeof params.shipping === 'object' && params.shipping.address) {
      // Clean up shipping address to only include Stripe-allowed fields
      const cleanAddress = {
      ...
      Object.keys(cleanAddress).forEach(key => {
        if (cleanAddress[key as keyof typeof cleanAddress] === undefined || cleanAddress[key as keyof typeof cleanAddress] === null || cleanAddress[key as keyof typeof cleanAddress] === '') {
          delete cleanAddress[key as keyof typeof cleanAddress];
        }
      });

      createParams.shipping = {
        name: params.shipping.name || params.shipping.address.name || 'Customer',
...
export async function updatePaymentIntent(
  ...
  try {
    ...
    if (params.shipping && typeof params.shipping === 'object' && params.shipping.address) {
      ...
      updateData.shipping = {
        name: params.shipping.name || 'Customer',
        address: cleanAddress
      };
    }
...
export async function calculateTax(params: {
  ...
  try {
    // Skip Stripe Tax API in development for faster performance
    if (process.env.NODE_ENV !== 'production') {
      console.log('⚡ Skipping Stripe Tax API in development');
      return null;
    }
